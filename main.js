/*----Global variabels----*/
var update_time = 1000; // ms
var osrm_base_url = "http://193.205.83.99:5000/route/v1/driving/";
...
var current_position = [];          //it is a matrix 5x4, where each row is a car and the columns = 0:id - 1:lat - 2:long - 3:angle
...
vec[id_emergency] = [ //vector with the coordanate points of the Emergency Vehicle (emergency)
  {
...
/// Function to update Markers in the map
function updateMarker(id, lat, lng, cardinality) {
  if (markers_objs[id]) { // Existing objects
    markers_objs[id].setLatLng([lat, lng]);
    if (id == id_emergency) {                      // id_emergency
      console.log("Emerg.: " + (lat).toFixed(6) + "," + (lng).toFixed(6));
      markers_objs[id].setIcon(emergencyIcon);
    } else {                                       // Vehicles
      console.log('Car[' + id + ']: ' + (lat).toFixed(6) + ',' + (lng).toFixed(6) + ',' + (cardinality).toFixed(0) + '°');
      setObjCardinality(id, cardinality);
    }
  } else { // Not existing objects
    var marker_obj = L.marker([lat, lng], { icon: ((id == id_emergency) ? emergencyIcon : black_N) });
    markers_objs[id] = marker_obj;
    setObjCardinality(id, cardinality, 0);
    current_position[id][1] = lat;
    current_position[id][2] = lng;
    console.log('New Vehicle: id=' + id + ' at ' + (lat).toFixed(6) + "," + (lng).toFixed(6));
    markers_objs[id].addTo(map);
  } // end if/else id
} //end function updateMarker()

/// Function to select (by id) 'arrow icon' to represent the car in the map
function setObjCardinality(id, cardinality) {          //id:= which car      cardinality:= angle
  ...
  else markers_objs[id].setIcon(emergencyIcon);
...
function track() {
  ...
  //creating a matrix 5x4 to hold the current position of the devices [each row is a car, the columns = 0:id - 1:lat - 2:long - 3:angle]
  for (var k = 0; k < 5; k++) {
    current_position[k] = new Array(4);
    current_position[k][0] = k;
  }

  /// Function to treat the json with the coordinates
  function updateMap() {
    setTimeout(function () {
      ...
      for (var k = 4; k >= 0; k--) {
        ...
        if (i[k] < Object.keys(vec[k]).length) {
          ...
          //if it is the emergency vehicle
          if (k == id_emergency) {
            if (current_position[id_emergency][1] != undefined) {
              VRU_lat = (current_position[id_emergency][1]).toFixed(6);
            } else {
              VRU_lat = "";
            }
            if (current_position[id_emergency][2] != undefined) {
              VRU_lon = (current_position[id_emergency][2]).toFixed(6);
            } else {
              VRU_lon = "";
            }
            document.getElementById("VRU").innerHTML = "Lat.: " + VRU_lat + "</br>Lon.: " + VRU_lon;
          } else { // if it is a car
            if ((vec[k][i[k]].lat != undefined) && (vec[k][i[k]].lon != undefined)) {
                f_servready = 1;              //flag indicating to wait the server return before continue
              osrm_query(k, vec[k][i[k]].lat, vec[k][i[k]].lon, vec[id_emergency][i[id_emergency]].lat, vec[id_emergency][i[id_emergency]].lon);
          ...
          if (i[k] != 0) {
            ...
            ddist = CalcDistance(current_position[k][1], current_position[k][2], vec[k][i[k]].lat, vec[k][i[k]].lon);
            if(ddist != 0)
              angle = -(Math.atan2(delta_x, delta_y) * 180 / Math.PI - 90);  //0 = north (clockwise is positive; counterclockwise is negative; from -90 to 270)
            else
              angle = current_position[k][3];
          }

          //calling function updateMarker()
          updateMarker(k, vec[k][i[k]].lat, vec[k][i[k]].lon, angle);

          //update the current position saved in the matrix
          current_position[k][1] = vec[k][i[k]].lat;
          current_position[k][2] = vec[k][i[k]].lon;
          current_position[k][3] = angle;
      ...
      //if it didn't reach the end of the vector return to beginning of the function for the next iteration
    ...
    }, update_time) //execute the loop every 'update_time'
...
///Function to query OSRM and update the information on HTML page
